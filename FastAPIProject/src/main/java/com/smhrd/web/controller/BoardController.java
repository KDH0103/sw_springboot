...
public class BoardController {
	...
	public String board(Model model){
		...
		return "board";
		// --> 이동해야할 페이지를 리턴
		// ViewResolver 논리적인 주소값을 물리적 주소값으로 변환
	...
	public String insert(Board board) {
		...
		// 2. board.jsp로 이동(화면에 추가한 게시글이 보여야흠)
		return "redirect:/";  
	}
	//게시글 한개 상세보기 기능
	@GetMapping("/selectOne/{bnum}")
	public String selectOne(@PathVariable int bnum, Model modle) {
		// 1. DB에서 게시글 한개에 대한 정보를 조회
		Board b = mapper.selectOne(bnum); 
		// 2. 조회한 정보를 request 영역에 담아주기
		modle.addAttribute("board",b);
		// 3. selectOne이라는 페이지로 forward 이도
	    return "selectOne";
	}
	
	@GetMapping("/count")
	@ResponseBody //--> 페이지를 이동하는 메소드가 아니라. 
	//비동기 통신을 처릴하는 메소드임을 나태는 annotation
	...
	public String temp(RedirectAttributes rttr) {
		...
		// 문자열만 저장 가능하다. (url에 quertString 형식으로 전송되는거라 한게가 있음 ㅠㅠ)
