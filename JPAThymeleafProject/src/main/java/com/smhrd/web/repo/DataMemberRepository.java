...
//JPA 사용시, 대상이 되는 테이블 내부에 PK가 정수형태라면?
...
public interface DataMemberRepository extends JpaRepository<DataMember, String> {
	...
	// hibernate가 메소드명칭을 보고 -> sql구문을 제작
	// 기본으로 제공되는 메소드
	// 1. find()
	// : SELECT * FROM DATAMEMBER;
	// 2. findById() >> PK기준으로 조회할 때 사용
	// *** 여러개 단어를 하나로 작성할 때, 반드시 카멜식 기법 따라야한다.
	// 3. save(DataMember m)
	...
	// --> JPA 규칙을 지켜주자!
	// find ... : select 구문동작 >> ... 컬럼이름들이 들어올 수 있음
	// By : 조건을 부여하겠다. (where)
